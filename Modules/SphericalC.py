...
def predictive_check(G, samples_Z, samples_a, samples_b):
    """
    ...
    for l in range(num_samples):
        ...
        for i in range(samples_Z.shape[1]):
            for j in range(samples_Z.shape[1]):
                if i != j:
                    dist = samples_Z[l,i,:].T @ samples_Z[l,j,:]
                    P[i, j] = expit(samples_a[l] + samples_b[l]*dist)
...
def plot_pairwise_sociomatrix(G,samples_Z, samples_a, samples_b, A_real):
    """
    ...
    for l in range(num_samples):
        for i in range(n):
            for j in range(n):
                if i != j:
                    dist_sq = samples_Z[l,i,:].T @ samples_Z[l,j,:]
                    probs_samples[l, i, j] = expit(samples_a[l] + samples_b[l]* dist_sq)
...
def compute_log_likelihood(A_obs, samples_Z, samples_a, samples_b):
    """
    ...
    for l in range(L):
        for i in range(n):
            for j in range(n):
                if i != j:
                    dist_sq = samples_Z[l,i,:].T @ samples_Z[l,j,:]
                    p_ij = expit(samples_a[l] + samples_b[l]* dist_sq)
                    if A_obs[i, j] == 1:
...
def compute_DIC_S1(log_lik, A_obs, samples_Z, samples_a,samples_b):
    """
    ...
    angles_mean = circmean(np.arctan2(samples_Z[:, :, 1], samples_Z[:, :, 0]), high=2*np.pi, low=0,axis=0)
    Z_mean =  np.stack((np.cos(angles_mean), np.sin(angles_mean)), axis=1)
    a_mean = np.mean(samples_a)
    b_mean = np.mean(samples_b)
    ...
    for i in range(n):
        for j in range(n):
            if i != j:
                dist_sq = Z_mean[i, :].T @ Z_mean[j, :]
                p_ij = expit(a_mean + b_mean * dist_sq)
                if A_obs[i, j] == 1:
...
def residual_analysis(A_obs, samples_Z, samples_a, samples_b):
    import numpy as np
    import seaborn as sns
    import matplotlib.pyplot as plt
    from scipy.special import expit
...
def residual_distributions(A_obs, samples_Z, samples_a, samples_b):
    """
    ...
    Parameters:
        ...
        samples_b (np.ndarray): Scaler samples (L,)
...
def posterior_connection_probs(samples_Z, samples_a, samples_b):
    """
...
def simulate_networks(samples_Z, samples_a, samples_b):
    n = samples_Z.shape[1]
    prob_matrices = []
    for l in range(len(samples_a)):
        ...
        for i in range(n):
            for j in range(n):
                if i != j:
                    dist_sq = samples_Z[l,i,:].T @ samples_Z[l,j,:]
                    P[i, j]  = expit(samples_a[l] + samples_b[l] * dist_sq) 
...
def modularity_check(A_obs, samples_Z, samples_a, samples_b):
    n = A_obs.shape[0]
    ...
    A_simulated = simulate_networks(samples_Z, samples_a, samples_b)
