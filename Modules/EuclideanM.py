    ...
    for iter in tqdm(range(number_of_iterations)):
        ...
        if adapting and iter > 0:
            ...
            if current_accept_rate < 0.80:
                epsilon = np.max(np.array([0.01,0.995*epsilon])) 
                std_dev_Z = np.max(np.array([0.05,0.995*std_dev_Z]))
            elif current_accept_rate > 0.60:
                epsilon = np.min(np.array([0.2,1.005*epsilon]))
                std_dev_Z = np.min(np.array([1.75,1.005*std_dev_Z]))
            L = max(1, int(round(1/epsilon))) 
        elif iter == warmup:
            print(f"Final parameters: epsilon={epsilon:.4f}, L={L}, std_dev_Z={std_dev_Z:.4f}, std_dev_a={std_dev_a:.4f}")
